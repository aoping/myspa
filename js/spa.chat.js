spa.chat=(function(){
	...
	var
		...
		setJqueryMap = function(){
		    ...
		    jqueryMap = {
		      ...
		      $close   : $slider.find( '.spa-chat-closer' ),
		...
		setSliderPosition = function(position_type,callback){
			...
			switch(position_type){
				case 'opened' :
					...
					animate_time = stateMap.slider_opened_time;
	...
	writeChat = function ( person_name, text, is_user ) {
	  ...
	  clearChat = function () { jqueryMap.$msg_log.empty(); };
		...
		onTapToggle = function(do_hidden){
  ...
  onUpdatechat = function ( event, msg_map ) {
    ...
    if ( is_user ) {
  	...
  	onLogin = function ( event, login_user ) {
	    configMap.set_chat_anchor( 'closed' ); // 防止一开始就是opened的情况
	  ...
	  onLogout = function ( event, logout_user ) {
		...
		initModule = function($append_target){
		    ...
		    jqueryMap.$head.click( function(e){
		    	onTapToggle();
		    } );
		    jqueryMap.$close.click( function(e){
		    	onTapToggle(true);
		    } );
